...
public class LunarCalendar {
    ...
    final static String chineseMonth[] = {"正", "二", "三", "四", "五", "六", "七", "八", "九", "十", "冬", "腊"};
    final static String chineseDay[] = {
    ...
    public static int[] lunarToSolar(int year, int month, int monthDay,
        ...
        if (year < MIN_YEAR || year > MAX_YEAR || month < 1 || month > 12
            ...
            LogUtils.d(TAG, year + "_" + month + "_" + monthDay);
    ...
    public int[] getSolar(int year, String Chinesedate) {
        int iYear, iMonth, iDay;
        boolean isLeapMonth = false;
        iYear = year;
        int i;
        LogUtils.d(TAG, "Chinese date = " + year + " "+ Chinesedate);
        if (Chinesedate.substring(0, 1).equals("闰")) {
            isLeapMonth = true;
            iMonth = leapMonth(iYear);
        } else {
            for (i = 0; i < 12; i++) {
                if (Chinesedate.substring(0, 1).equals(chineseMonth[i])) {
                    break;
                }
            }
            iMonth = i + 1;
        ...
        if (Chinesedate.length() == 3) {
            if (Chinesedate.substring(2, 3).equals(chineseDay[20 - 1])) {
                iDay = 20;
            } else {
                iDay = 30;
            }
            ...
            for (i = 0; i < 30; i++) {
                if (Chinesedate.substring(2, 4).equals(chineseDay[i])) {
                    break;
                }
            }
            iDay = i+1;
        }
        int iDate[] = lunarToSolar(iYear, iMonth, iDay, isLeapMonth);
        LogUtils.d(TAG, iDate[0] + " , " + iDate[1] + " , " + iDate[2]);
        return iDate;
    }

    public int[] getSpring_Festival(int year) {
        int[] date = new int[2];
        date[0] = ((LUNAR_INFO[year - MIN_YEAR] & 0x60)) >> 5;
        date[1] = ((LUNAR_INFO[year - MIN_YEAR] & 0x1F));
        return date;
    }
    ...
    public int[] solarToLunar(int year, int month, int monthDay) {
        ...
        c1.set(year, month - 1, monthDay);
        Calendar c2 = (Calendar) c1.clone();
        c2.set(1900, 0, 31);

        if (c1.before(c2)) {
            LogUtils.d(TAG, "can't convert before than 1900-1-31");
            return lunarDate;
        }
        iYear = year;
        int[] spring_festival = getSpring_Festival(year);
        c2.set(year, spring_festival[0] - 1, spring_festival[1]);
        if (c1.before(c2)) {
            iYear--;
            spring_festival = getSpring_Festival(year - 1);
            c2.set(year - 1, spring_festival[0] - 1, spring_festival[1]);
        }
        int offset = (int) ((c1.getTimeInMillis() - c2.getTimeInMillis()) / (24 * 60 * 60 * 1000));
    ...
    public String GetLunar(Calendar cal) {
        int[] date = solarToLunar(cal.get(Calendar.YEAR), cal.get(Calendar.MONTH) + 1,
                cal.get(Calendar.DAY_OF_MONTH));
        if (date[3] == 0) {
            return chineseMonth[date[1] - 1] + "月" + chineseDay[date[2] - 1];
        } else {
            return "闰月" + chineseDay[date[2] - 1];
        }
    }
